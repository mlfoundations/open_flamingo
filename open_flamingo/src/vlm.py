...
class Omnivore(nn.Module):
    ...
    Generic vision-language model (VLM) class.
    A VLM consists of four components:
        1. A vision tokenizer that extracts features from pixels, e.g. VQGAN
            input: (B, T_img, F, C, H, W)
            output: (B, T_img, d)
        2. A fusion method that allows the language model to attend to these tokens, e.g. cross-attention, or placing the tokens directly in the language model's input sequence
        3. A language model
    """
    ...
    def __init__(
        ...
        """
        Args:
            vision_encoder (nn.Module): e.g. VQGAN
            lang_model (nn.Module): e.g. MPT
            initial_tokenizer_len (int): size of the original tokenizer vocab
            pad_token_id (int): id of the pad token
        ...
        self.num_tokens_per_vis = vision_encoder.num_tokens_per_media
        self.initial_tokenizer_len = initial_tokenizer_len 
        # core components
        self.vision_encoder = vision_encoder
        self.lang_model = lang_model
        
        for i in range(self.vision_encoder.config.num_embeddings):
            self._special_tokens[f"image_token_{i}"] = f"<image_{i}>"
    ...
    def _prepare_inputs_for_forward(
        ...
        for i in range(B):
            ...
            if len(image_token_idxs) == 0:
                multimodal_input_ids.append(lang_x[i].clone())
            ...
            for img_num, img_idx in enumerate(image_token_idxs):
                new_input_ids = torch.cat(
                    (
                        new_input_ids[:img_idx],
                        vision_tokens[i, img_num].squeeze(0),
                        new_input_ids[img_idx + 1 :],
                    ),
                    dim=0,
                )
                ...
                if has_labels:
                    new_label = torch.cat(
                        (
                            new_label[:img_idx],
                            vision_tokens[i, img_num].squeeze(0).clone(),
                            new_label[img_idx + 1 :],
                        ),
                        dim=0,
                    )
            multimodal_input_ids.append(new_input_ids)
            multimodal_attention_mask.append(new_attention_mask)
            if has_labels:
                multimodal_labels.append(new_label)

        # stack
        multimodal_input_ids = stack_with_padding(
            multimodal_input_ids, padding_value=self.pad_token_id, padding_side="left" if generating else "right"
        )
    ...
    def forward(
        ...
        **kwargs,
        ...
        # convert pixels to vision tokens
        if vision_x is not None:
            vision_tokens = self._encode_vision_x(vision_x=vision_x)
        else:
            vision_tokens = None
        ...
        # print("input_ids", new_inputs["input_ids"][0])
        # print("attention_mask", new_inputs["attention_mask"][0])
        # print("labels", new_inputs["labels"][0])
    ...
    def _encode_vision_x(self, vision_x: torch.Tensor):
        ...
        vision_x = rearrange(vision_x, "b T F c h w -> (b T F) c h w")
        with torch.no_grad():
            vision_x = self.vision_encoder.encode(vision_x)[1]
        for seq_idx in range(vision_x.shape[0]):
            for token_idx in range(vision_x.shape[1]):
                vision_x[seq_idx][token_idx] = getattr(self, f"image_token_{vision_x[seq_idx][token_idx]}_id")
        vision_x = rearrange(vision_x, "(b T F) t -> b T F t", b=b, T=T, F=F)
