...
def remove_punctuation_based_sentences(interleaved_list, is_image_list):
    ...
    """
    for i, (interleaved_input, is_image) in enumerate(
        zip(interleaved_list, is_image_list)
    ):
        if not is_image:
...
def is_tiny_image(image):
    ...
    """
    return (
        image.size[0] <= TINY_IMAGE_SIZE_THRESHOLD
        or image.size[1] <= TINY_IMAGE_SIZE_THRESHOLD
    )
...
def preprocess_sentences(interleaved_list, is_image_list):
    ...
    interleaved_list, is_image_list = remove_punctuation_based_sentences(
        interleaved_list, is_image_list
    )
    interleaved_list, is_image_list = filter_out_empty_sentences(
        interleaved_list, is_image_list
    )
    assert len(interleaved_list) == len(
        is_image_list
    ), "lengths of the interleaved and is_image list should be same"
    return len(interleaved_list), interleaved_list, is_image_list


def preprocess_interleaved_sample(
    interleaved_list, is_image_list, images, text_tokenizer, image_processor
):
    num_images, interleaved_list, is_image_list = remove_unwanted_images(
        interleaved_list, is_image_list, images
    )
    if num_images == 0:
        raise ValueError("No images in sample")

    num_sentences, interleaved_list, is_image_list = preprocess_sentences(
        interleaved_list, is_image_list
    )
    if num_sentences == 0:
        raise ValueError("No sentences in sample")

    images, interleaved_list, is_image_list = substitute_with_image_tag(
        interleaved_list, is_image_list, images
    )
...
def preprocess_interleaved_data(data, text_tokenizer, image_processor):
    ...
    images = {
        ...
        if k.endswith("png") or k.endswith("jpg") or k.endswith("jpeg")
    }
    images_tensor, (
        text_input_ids,
        text_attention_mask,
    ) = preprocess_interleaved_sample(
        interleaved_list, is_image_list, images, text_tokenizer, image_processor
    )
